...
{
    memset(o, 0, sizeof(*o));
    o->rate = DEFAULT_RATE_PPS;

    int c;
    while ((c = getopt(argc, argv, "p:d:r:i:t:X")) != -1) {
        switch (c) {
        ...
        case 't': strncpy(o->target, optarg, sizeof(o->target) - 1); break;
        case 'X': o->af_xdp = true; break;
        default: exit(1);
        }
    }

    if (!o->port || !o->duration || !o->target[0]) {
        fprintf(stderr, "Usage: -p port -d sec -t target [-i iface] [-X]\n");
        exit(1);
    }
}
...
int main(int argc, char **argv)
{
    ...
    if (o.af_xdp) {
        printf("[!] AF_XDP richiesto, ma su questa macchina potrebbe non essere supportato\n");
        printf("[!] Fallback automatico a UDP\n");
    }
